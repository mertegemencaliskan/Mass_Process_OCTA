...
template_crop_image_path = 'path to the image that you want to crop'


img = cv2.imread(template_crop_image_path)
img_dup = np.copy(img)
mouse_pressed = False
...
path_output = 'User Will Enter Path for desired folder'
path_input = 'User Will Enter Path for desired folder'
dirs = os.listdir(path_input)

def crop():
    for item in dirs:
        fullpath = os.path.join(path_input,item)         #corrected
        if os.path.isfile(fullpath):
            im = Image.open(fullpath)
            f, e = os.path.splitext(fullpath)
            imCrop = im.crop((starting_x, ending_x, starting_y, ending_y)) #corrected
            imCrop.save(os.path.join(path_output, f + 'Cropped.bmp'), "BMP", quality=100)
